...
export default function Home() {
  ...
  // Fetch members from Firestore and pick 15 random
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        ...
        // Shuffle and pick 15 random
        const shuffled = [...membersData];
        for (let i = shuffled.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
        }
        const randomFifteen = shuffled.slice(0, 15);
        setMembers(randomFifteen);
      } catch (error) {
      ...
      <main className="bg-black text-white">
        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
          <div 
            ...
            style={{ 
              backgroundImage: "url('https://images.unsplash.com/photo-1516589091380-5d8e4f6d40ca?q=80&w=2070&auto=format')",
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/signup">
                  <button className="bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white font-semibold py-4 px-8 rounded-full text-lg shadow-lg transition-all flex items-center gap-2 mx-auto">
                    Find Your Match <FontAwesomeIcon icon={faArrowRight} />
                  </button>
                </Link>
        ...
        {/* Member Grid – 15 random profiles */}
          ...
          <div className="max-w-7xl mx-auto">
            ...
            >
              <h2 className="text-3xl md:text-4xl font-bold mb-3">
                Meet Beautiful <span className="text-rose-400">Singles</span>
              </h2>
              <p className="text-gray-400 max-w-xl mx-auto">
                {loading ? "Loading members..." : `Showing 15 random profiles from our community`}
              </p>
            </motion.div>
            ...
            <div className="text-center mt-12">
              <Link to="/explore">
                <button className="border border-white/30 hover:bg-white/10 rounded-full py-3 px-8 font-medium transition inline-flex items-center gap-2">
                  Explore Other Singles <FontAwesomeIcon icon={faUserPlus} />
                </button>
              </Link>
            </div>
          </div>
